...
# This example includes 100 random samples, 1000 CpGs, and 1000 genes sourced from 
...
# Load data
metMat <- read.table("/data/HumanMethylation450_TCGA-PANCAN_1000x1000.txt", sep = "\t")   # Methylation data
exprMat <- read.table("/data/HTSeq-FPKM-UQ_TCGA-PANCAN_1000x1000.txt", sep = "\t")        # Expression data
probeinfo <- read.table("/data/Probeinfo.txt", sep = "\t")                                # CpG location file

# Match sample names in the columns and check same order
metMat <- metMat[, match(colnames(exprMat), colnames(metMat))]
all(colnames(metMat) == colnames(exprMat))

# Remove CpGs located at the sex chromosomes
...
condor.object <- condorCluster(condor.object, cs.method = "LCS")

# Make dataframes with CpGs and genes with community annotations
geneinfo <- condor.object$blue.memb; geneinfo
probeinfo <- condor.object$red.memb; probeinfo
